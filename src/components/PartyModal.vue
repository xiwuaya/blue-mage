...
const removeTeammate = () => {
  if (localPartyData.value.length > 0) {
    ...
    localPartyVisibilityStates.value.pop(); 
  }
};
        ...
        <div class="modal-content party-modal">
          ...
          <div class="help-text">
            ...
            <div class="party-grid">
              ...
              <div class="party-user" :class="{ 'layer-hidden': localUser1VisibilityState === 2 }">
                <div class="name-row">
                  ...
                  <button 
                    ...
                    :class="{ 'is-must-include': localUser1VisibilityState === 1 }"
                  ...
                  >
                    <span class="eye-icon" :class="localUser1VisibilityState === 2 ? 'icon-invisible' : 'icon-visible'"></span>
                  </button>
                </div>
              ...
              <div 
                class="party-user" 
                :class="{ 'layer-hidden': localPartyVisibilityStates[index] === 2 }" 
                v-for="(data, index) in localPartyData" 
                :key="index"
              >
                <div class="name-row">
                  ...
                  <button 
                    class="visibility-btn" 
                    :class="{ 'is-must-include': localPartyVisibilityStates[index] === 1 }"
                  ...
                  >
                    <span class="eye-icon" :class="localPartyVisibilityStates[index] === 2 ? 'icon-invisible' : 'icon-visible'"></span>
                  </button>
                </div>
...
.help-text a { color: #ffbe31; text-decoration: underline; }
...
.name-row { display: flex; align-items: center; margin-bottom: 5px; gap: 5px; }
.name-input { flex: 1; background: transparent; border: 1px dashed transparent; color: #ffbe31; font-size: 0.9rem; font-weight: bold; padding: 0 4px; border-radius: 4px; height: 28px; width: 100%; box-sizing: border-box; outline: none; }
...
.party-user textarea { width: 100%; box-sizing: border-box; background: #1a1a1a; color: #fff; border: 1px solid #444; border-radius: 4px; padding: 8px; resize: vertical; min-height: 60px; font-family: monospace; font-size: 0.85rem; }
.party-user textarea:focus { outline: none; border-color: #ffbe31; }
.eye-icon { width: 18px; height: 18px; display: inline-block; background-color: currentColor; }
.icon-visible { -webkit-mask: url('/icons/Visible.svg') no-repeat center / contain; mask: url('/icons/Visible.svg') no-repeat center / contain; }
...
.layer-hidden textarea { color: #777 !important; border-color: #333 !important; }
.layer-hidden .visibility-btn { color: #777; }
.copy-btn { position: absolute; top: 6px; right: 6px; background: #2b2b2b; color: #ffbe31; border: 1px solid #444; border-radius: 4px; padding: 2px 8px; font-size: 0.75rem; font-weight: bold; cursor: pointer; opacity: 0.7; }
.copy-btn:hover { opacity: 1; background: #ffbe31; color: #1a1a1a; border-color: #ffbe31; }
.copy-toast { position: absolute; top: 36px; right: 6px; background-color: #ffbe31; color: #1a1a1a; padding: 4px 8px; border-radius: 4px; font-size: 0.75rem; font-weight: bold; pointer-events: none; box-shadow: 0 2px 6px rgba(0, 0, 0, 0.4); z-index: 10; }
.reset-wrap { margin-top: 20px; display: flex; justify-content: flex-end; }
.reset-btn { background-color: transparent; color: #ffbe31; border: 1px solid #ffbe31; border-radius: 4px; padding: 6px 16px; font-size: 0.9rem; font-weight: bold; cursor: pointer; transition: all 0.2s; }
.reset-btn:hover { background-color: #ffbe31; color: #1a1a1a; }
.filter-header { font-size: 1rem; font-weight: bold; color: #ffbe31; cursor: pointer; margin-bottom: 10px; display: flex; align-items: center; }
...
.user-badge { display: inline-block; background: #ffbe31; color: #1a1a1a; padding: 2px 10px; border-radius: 12px; margin-right: 6px; font-size: 0.8rem; font-weight: bold; }
...
.action-btn { flex: 1; background: transparent; border: 1px dashed #444; color: #ccc; border-radius: 4px; cursor: pointer; font-size: 0.9rem; font-weight: bold; transition: all 0.2s; }
